  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        className="group -mx-1.5 -my-0.5 flex max-w-[60vw] items-center gap-1.5 rounded-lg px-1.5 py-0.5 text-left transition-colors hover:bg-sage-100 sm:max-w-none"
      >
        ...
        <h1 className="truncate text-base font-semibold tracking-tight text-sage-900">
          {active?.name ?? 'Weekly Planner'}
        </h1>
        <ChevronDown
          className={`h-4 w-4 shrink-0 text-sage-400 transition-transform group-hover:text-sage-600 ${
            open ? 'rotate-180' : ''
          }`}
        />
      </button>
      ...
      {open && (
        <div
          ...
          className="absolute left-0 top-full z-40 mt-2 w-72 overflow-hidden rounded-2xl border border-sage-200 bg-paper/95 backdrop-blur-sm shadow-[0_2px_6px_rgba(40,48,40,0.06),0_20px_44px_-24px_rgba(40,48,40,0.4)]"
        >
          <p className="px-3 pt-2.5 pb-1 text-[10px] font-semibold uppercase tracking-wider text-sage-400">
            Planners
          </p>
  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        className="inline-flex items-center gap-1.5 rounded-full border border-sage-200 px-3 py-1.5 text-xs font-medium text-sage-600 transition-colors hover:border-sage-300 hover:bg-sage-100"
      >
      ...
      {naming && (
        ...
        <div className="absolute right-0 top-full z-40 mt-2 w-72 rounded-2xl border border-sage-200 bg-paper/95 backdrop-blur-sm p-3 shadow-[0_2px_6px_rgba(40,48,40,0.06),0_20px_44px_-24px_rgba(40,48,40,0.4)]">
          <NewPlannerForm
...
function NewPlannerForm({
  ...
  return (
    <div className="space-y-3">
      <div>
        <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-sage-400">
          Name
        </label>
        <input
          autoFocus
          ...
          className="w-full rounded-lg border border-sage-300 px-2.5 py-1.5 text-sm text-sage-900 outline-none placeholder:text-sage-400 focus:border-moss-500"
        />
      </div>
        ...
        {custom && (
          <div className="mt-2 flex items-center gap-2">
            <input
              ...
              className="w-20 rounded-lg border border-sage-300 px-2.5 py-1.5 text-sm text-sage-900 outline-none focus:border-moss-500"
            />
      ...
      <div className="flex items-center justify-end gap-1.5">
        <button
          onClick={onCancel}
          className="rounded-lg px-2.5 py-1.5 text-xs font-medium text-sage-500 transition-colors hover:bg-sage-100 hover:text-sage-800"
        >
          Cancel
        </button>
        <button
          onClick={submit}
          disabled={!valid}
          className="rounded-lg bg-moss-600 px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-moss-500 disabled:cursor-not-allowed disabled:opacity-40"
        >
          Create planner
        </button>
      </div>
...
function PlannerRow({
  ...
  return (
    ...
    >
      ...
      >
        {isActive ? (
          <Check className="h-4 w-4 shrink-0 text-sage-900" />
        ) : (
          <span className="h-4 w-4 shrink-0" />
        )}
        <span
          className={`truncate text-sm ${
            isActive ? 'font-semibold text-sage-900' : 'text-sage-700'
          }`}
        >
          {planner.name}
        </span>
        ...
        {planner.day_count !== DEFAULT_PLANNER_DAYS && (
          <span className="shrink-0 text-[10px] font-medium text-sage-400">
...
function RowAction({
  ...
  return (
    <button
      ...
      title={label}
      aria-label={label}
      className={`rounded-md p-1.5 text-sage-400 transition-colors disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-transparent disabled:hover:text-sage-400 ${
        danger ? 'hover:bg-red-100 hover:text-red-600' : 'hover:bg-sage-200 hover:text-sage-700'
      }`}
    >
  ...
  return (
    <div className="flex items-center gap-1 py-0.5">
      <input
        ...
        onBlur={submit}
        className="min-w-0 flex-1 rounded-lg border border-sage-300 px-2 py-1.5 text-sm text-sage-900 outline-none focus:border-moss-500"
      />
      <button
        ...
        className="rounded-md p-1.5 text-sage-400 transition-colors hover:bg-sage-100 hover:text-sage-700"
        aria-label="Cancel"
      >
        <X className="h-3.5 w-3.5" />
      </button>
